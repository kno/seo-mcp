...
Google Ads has no per-site notion at all (`src/google/ads.ts` takes only `customerId`), so an Ads probe can never prove "access to _this site's_ data". Blocking a whole site's dashboard because Keyword Planner is not enabled would make eleven working views unreachable for a keyword-research failure. Ads health is therefore stored and displayed per `(site, "google-ads")` but does not affect selectability; the Ads tools surface `upstream_source_not_configured` on their own when Ads is unhealthy. **This refines the user's directive** — flagged, not assumed silently.
